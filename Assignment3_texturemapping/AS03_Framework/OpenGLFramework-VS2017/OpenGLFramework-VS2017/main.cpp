...
// <SELF ADD HW2>
ModeType cur_mode_type = Transformation;
...
// <SELF ADD HW3>
int mag_texture_filtering_mode = 0;
int min_texture_filtering_mode = 0;
...
// <SELF ADD HW2> compute phong model
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	{	
		// <SELF ADD HW3> set eye offset
		GLfloat eye_offset[2] = { 0.0f, 0.0f };
		if (models[cur_idx].shapes[i].material.isEye) {
			eye_offset[0] = 0.5f * (models[cur_idx].cur_eye_offset_idx / 4);
			eye_offset[1] = -0.25f * (models[cur_idx].cur_eye_offset_idx % 4);
		}
		glUniform2fv(iLocEyeOffset, 1, eye_offset);


		phong_model(i, view_matrix, per_vertex_or_per_pixel);
		...
		if (mag_texture_filtering_mode == 0) glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		else if (mag_texture_filtering_mode == 1) glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

		if (min_texture_filtering_mode == 0) glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		else if (min_texture_filtering_mode == 1) glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		GLint tmp_eye_offset_idx = models[cur_idx].cur_eye_offset_idx;
		GLint tmp_max_eye_offset = models[cur_idx].max_eye_offset;
		...
		case GLFW_KEY_I:
			...
			break;
		case GLFW_KEY_G:
			mag_texture_filtering_mode = (mag_texture_filtering_mode == 0) ? 1 : 0;
			break;
		case GLFW_KEY_B:
			min_texture_filtering_mode = (min_texture_filtering_mode == 0) ? 1 : 0;
			break;
		case GLFW_KEY_LEFT:
			models[cur_idx].cur_eye_offset_idx = (tmp_eye_offset_idx - 1 + tmp_max_eye_offset) % tmp_max_eye_offset;
			break;
		case GLFW_KEY_RIGHT:
			models[cur_idx].cur_eye_offset_idx = (tmp_eye_offset_idx  + 1) % tmp_max_eye_offset;
			break;
		default:
			break;
		}
				...
				if (cur_light_mode == LightEdit) {
					...
					case Directional:
						Light.directional.position +=
							Vector3(TICK * -diff_x, TICK * diff_y, 0);
						break;
					case Point:
...
void LoadTexturedModels(string model_path)
{
	...
	for (int i = 0; i < materials.size(); i++)
	{
		...
		// <SELF ADD HW3> to check whether the file is Eye
		material.isEye = (string(materials[i].diffuse_texname).find("Eye") != std::string::npos ) ? 1 : 0;
...
void setUniformVariables()
{
	...
	iLocEyeOffset = glGetUniformLocation(program, "eye_offset");
	...
	iLocMode = glGetUniformLocation(program, "cur_light_type");
	iLocDiffuse = glGetUniformLocation(program, "diffuse");
	iLocSpecular = glGetUniformLocation(program, "specular");
	iLocAmbient = glGetUniformLocation(program, "ambient");
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "109065539 HW3", NULL, NULL);
    if (window == NULL)
    {
